        ...
        <div className="flex flex-wrap gap-2">
          {mode === "edit" && problem ? (
            <>
              ...
              <Badge variant="flat">{judgeModeLabel(problem.judgeMode)}</Badge>
              {updatedAt ? <Badge variant="flat">更新于 {updatedAt}</Badge> : null}
            </>
          ) : (
            <>
              <Badge variant="flat">未持久化</Badge>
              <Badge variant="flat">创建模式</Badge>
            </>
          )}
        </div>
      </div>
