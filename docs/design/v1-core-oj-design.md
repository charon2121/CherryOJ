...
| 组件 | 责任 |
|---|---|
| `cherry-web` | 页面呈现、表单提交、状态轮询、权限入口控制 |
...
```text
+-------------+        HTTP         +-------------+        HTTP / RPC       +----------------+
|  cherry-web  | <-----------------> |   cherry    | <--------------------> |  cherry-judge  |
...
```text
1. cherry-web 发起 POST /api/submissions
...
8. cherry-web 轮询 GET /api/submissions/{id} 获取最终结果
```
...
详细字段、状态码、示例请求和响应见 [CherryOJ Judge Interface Contract v1](./v1-judge-interface-contract.md)。

核心契约原则：

- 判题服务返回“执行事实”
- 主后端负责把执行事实翻译为平台状态与业务展示字段
- 浏览器只调用 `cherry`，不直接调用 `cherry-judge`
- `cherry-judge` 不直接写主数据库，由 `cherry` 统一回写 `submission` 与 `submission_test_case_result`
...
前后端应统一响应壳。当前 `cherry-web/src/lib/api/types.ts` 中已有：
...
| 目标 | 仓库位置 |
|---|---|
| 前端题库与做题页面 | `cherry-web/src/components/oj` |
| 前端认证能力 | `cherry-web/src/components/auth` |
| 前端 API 封装 | `cherry-web/src/lib/api` |
