...
System
  系统配置（disabled / coming soon）
```

要求：

- 菜单参考 HeroUI dashboard template 的扁平 workspace 菜单，而不是旧式树状折叠菜单。
- active 状态清晰。
- 图标统一来自 `lucide-react`。
...
- 可以提供一行极短 secondary label，例如 `Problems`、`Create`
...
未来菜单项可以出现在已规划分组中，但必须保持 disabled / coming soon 状态，避免误导用户。
...
- 当前路径命中菜单项时，该项 active。
- 刷新页面后 active 状态必须由 pathname 推导，不依赖本地 store。
- 不做折叠展开，因此不需要保存 open state。
...
- `AdminSidebar.client.tsx` 可以是 Client Component，因为包含退出登录和路由交互。
...
- 已新增 `/admin/layout.tsx`
- 已新增 `/admin/providers.tsx`
- 已新增 `AdminShell.client.tsx`
- 已新增 `AdminSidebar.client.tsx`
- 已恢复 `/admin` 到 `/admin/problems` 的入口跳转
- 已按 `Sidebar + Content` 实现固定侧栏与内容滚动区域
- 已通过 `npm run typecheck` 和 `npm run lint`
